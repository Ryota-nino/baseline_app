...
const CompanyBar:React.FC<Props> = props => {
    ...
    const checkPrefLength = ()=> {
        if(companies[companyId]){
            ...
            const output = String(companies[companyId].pref.slice(0, MAX_LENGTH) + '...');
        ...
        <article className="company-bar">
            <div className="company-bar__wrap">
                ...
                <div className="company-bar__inner">
                    <h1 className="heading4">{companies[companyId] ? companies[companyId].company_name : ''}</h1>
                    <div>
                        <p className="company-bar__link">
                            <a target="_blank" href={companies[companyId] ? companies[companyId].url : ''}>
            ...
            <div className="company-bar__right">
                {props.thisPage === 'detail' ?  <Link to={`/company-insert/${props.companyId}`}>企業情報の編集</Link> : ''}
                {props.hasActionBtn && <ActionBtn type="button" txt="情報を追加する" isPlus={false} linkUrl={`company-info/${props.companyId}/`}/>}
            </div>
        </article>
    );
};
